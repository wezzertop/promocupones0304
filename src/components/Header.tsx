      ...
      <div className="hidden sm:block flex-1 w-full max-w-2xl relative group mx-1 md:mx-4 lg:mx-8">
        <form onSubmit={handleSearchSubmit} className="w-full relative">
          ...
          <input
            ...
            placeholder="Buscar..."
            autoComplete="off"
            className="w-full bg-surface text-foreground pl-9 pr-2 py-2.5 rounded-xl border border-border focus:outline-none focus:border-[#07B5A7]/50 focus:ring-1 focus:ring-[#07B5A7]/50 transition-[border-color,box-shadow] duration-200 placeholder:text-gray-600 text-sm relative z-0"
          />
          <div className="absolute right-3 top-2.5 hidden sm:flex items-center gap-1 pointer-events-none">
            <kbd className="hidden md:inline-flex h-5 items-center gap-1 rounded border border-border bg-surface-hover px-1.5 font-mono text-[10px] font-medium text-gray-500 opacity-100">
      ...
      <div className="hidden sm:flex items-center gap-1 sm:gap-4 ml-auto w-[100px] sm:w-auto justify-end">
        ...
        {isAdminOrMod && (
          <Link 
            ...
            className="hidden sm:flex items-center gap-2 px-3 py-2 bg-surface-hover/50 hover:bg-zinc-700/50 text-zinc-400 hover:text-foreground rounded-lg transition-colors border border-border"
        ...
        {user ? (
          <div className="relative">
            <button 
              ...
              className="flex items-center gap-3 pl-2 pr-1 py-1 rounded-xl hover:bg-surface transition-colors border border-transparent hover:border-border"
            >
              ...
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#07B5A7] to-[#25b84e] flex items-center justify-center text-black font-bold shadow-lg shadow-[#07B5A7]/20 overflow-hidden relative border-2 border-[#07B5A7]/30 group">
                ...
                <div className="absolute top-0 right-0 w-2.5 h-2.5 bg-[#07B5A7] rounded-full border-2 border-surface animate-pulse z-10 shadow-[0_0_8px_#07B5A7] transform translate-x-1/4 -translate-y-1/4"></div>
              </div>
            </button>
            ...
            {isUserMenuOpen && (
              <div className="absolute right-0 top-full mt-2 w-56 bg-surface border border-border rounded-xl shadow-2xl py-1 z-[100] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="p-1">
                  ...
                  <Link 
                    href="/perfil" 
                    className="flex items-center gap-3 px-3 py-2 text-sm text-foreground hover:bg-surface-hover hover:text-foreground rounded-lg transition-colors"
                    onClick={() => setIsUserMenuOpen(false)}
                  >
