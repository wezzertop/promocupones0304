...
export default function GoogleTermsModal({ isOpen, onClose, onAccept }: GoogleTermsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
      <div className="w-full max-w-md bg-[#18191c] border border-[#2d2e33] rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">
        {/* Header with gradient line */}
        <div className="h-1.5 w-full bg-gradient-to-r from-transparent via-[#2BD45A] to-transparent opacity-50"></div>
        
        <div className="p-8">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-[#2BD45A]/10 rounded-xl">
                <Shield size={24} className="text-[#2BD45A]" />
              </div>
          ...
          <div className="space-y-3 mb-8">
            {[
              ...
              >
                <div className="p-2 bg-zinc-800 rounded-lg group-hover:bg-[#2BD45A]/10 transition-colors">
                  <item.icon size={18} className="text-gray-400 group-hover:text-[#2BD45A]" />
                </div>
                <div className="flex-1">
                  <p className="text-sm font-medium text-gray-200 group-hover:text-white transition-colors">{item.label}</p>
                  <p className="text-[10px] text-gray-500">{item.desc}</p>
                </div>
                <ArrowRight size={14} className="text-gray-600 group-hover:text-[#2BD45A] group-hover:translate-x-0.5 transition-all" />
              </Link>
            ))}
          </div>
            ...
            <label className="flex items-start gap-3 cursor-pointer group">
              <div className="relative flex items-center mt-0.5">
                ...
                <div className={`w-6 h-6 rounded-lg border-2 transition-all flex items-center justify-center ${
                  accepted 
                    ? 'bg-[#2BD45A] border-[#2BD45A] shadow-[0_0_10px_rgba(43,212,90,0.4)]' 
          ...
          <div className="flex gap-3">
            ...
            <button
              disabled={!accepted}
              onClick={onAccept}
              className="flex-1 px-4 py-3 rounded-xl bg-[#2BD45A] hover:bg-[#25b84e] text-black font-bold transition-all shadow-lg shadow-[#2BD45A]/20 disabled:opacity-50 disabled:cursor-not-allowed text-sm flex items-center justify-center gap-2 group/btn"
            >
              Continuar
